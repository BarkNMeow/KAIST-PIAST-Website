...
let originalName = $('#bbs-file-summary').html()

$('#bbs-title').on('input', checkValid);
$('#bbs-composer').on('input', checkValid);
$('#bbs-file').on('change', function () {
    const exists = $(this)[0].files.length > 0;
    if (!exists) {
        pdfHandler = new Promise((resolve, reject) => { resolve(); });
    } else {
    ...
    $('#bbs-file-summary').html(file ? file.name : originalName);
    checkValid();
});
